...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "QR kodni skaner qilib ichidagi kodni yuboring (masalan: ECO001).\n"
        "1 ball = 100 so'm. 10 ball = 1000 so'm.\n\n"
        "Buyruqlar:\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "ℹ️ Yordam:\n\n"
        "1️⃣ Mahsulot kodingizni yuboring.\n"
...
async def cashout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if points >= 10:
        ...
        # 🔔 Admin uchun reminder faqat cashoutda
        await context.bot.send_message(
            chat_id=MY_ID,
            text=f"💸 Foydalanuvchi {user_id} pul olish uchun {points} ball yig‘di!"
        )
    else:
        await update.message.reply_text("⚠️ Pul olish uchun kamida 10 ball yig‘ish kerak.")
